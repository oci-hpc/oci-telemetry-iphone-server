// telemetry source object for the F1_T09_Lobby_Info

const dgram = require('dgram');
const server = dgram.createSocket('udp4');
const fs = require('fs');
var F1_T09_Lobby_InfoCreateDict = require('../oci-telemetry-esports-F122-client/example/F1_T09_Lobby_Info/F1_T09_Lobby_InfoCreateDict');
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T09 Lobby Info");
...
function F1_T09_Lobby_Info() {
	...
	//let rawDict = fs.readFileSync('../oci-telemetry-esports-F122-client/example/F1_T09_Lobby_Info/F1_T09_Lobby_Infodictionary.json')
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 09:");
			console.log(err);
			return;
		}
		...
		console.log("Packet 9 length", this.data[2].length);
		if(this.data[2].length != 506){
			console.log("Rouge message length, packet 09:");
			return;
		}
		
		this.messageCount = this.messageCount + 1;
		if(this.messageCount % 60 == 0){
			console.log("    Packet 09 messages received: ", this.messageCount);
	...
	server.on('error', (err) => {
		console.log(`F1_T09_Lobby_Info UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T09_Lobby_Info initialized--");
};


// to update every time new data comes in
F1_T09_Lobby_Info.prototype.generateRealtimeTelemetry = function () {
...
F1_T09_Lobby_Info.prototype.generateIntervalTelemetry = function () {
...
F1_T09_Lobby_Info.prototype.notify = function (point) {
...
F1_T09_Lobby_Info.prototype.listen = function (listener) {
...
F1_T09_Lobby_Info.prototype.SetFileTimestamp = function () {
...
F1_T09_Lobby_Info.prototype.clearHistoryObject = function (){
	console.log("Clear History Object - F1_T09_Lobby_Info");
...
F1_T09_Lobby_Info.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T09_Lobby_Info.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
  }
...
F1_T09_Lobby_Info.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T09_Lobby_Info_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T09_Lobby_Info_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		//server.send(command,59023, 'localhost')
		server.send(command,59023, '0.0.0.0')
