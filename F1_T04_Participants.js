// telemetry source object for the F1_T04_Participants

const dgram = require('dgram');
const server = dgram.createSocket('udp4');
const fs = require('fs');
var F1_T04_ParticipantsCreateDict = require('../oci-telemetry-esports-F122-client/example/F1_T04_Participants/F1_T04_ParticipantsCreateDict');
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T04 Participants");
...
function F1_T04_Participants() {
	...
	//let rawDict = fs.readFileSync('../oci-telemetry-esports-F122-client/example/F1_T04_Participants/F1_T04_Participantsdictionary.json')
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 04:");
			console.log(err);
			return;
		}
		...
		// Packet 04 is expected to be 199
		//console.log("Packet 4 length", this.data[2].length);
		if(this.data[2].length != 199){
			console.log("Rouge message length, packet 04:");
			return;
		}
		
		this.messageCount = this.messageCount + 1;
		if(this.messageCount % 6 == 0){
			console.log("    Packet 04 messages received: ", this.messageCount);
		...
		if(this.data[0] == "packet_04"){
			...
			for (let i = 0; i < slugSize; i++){
				// Update current state
				let key = this.orderedKeys[i + 1];
				let newValue = this.data[2][player1Index * slugSize + i + 1];
				this.state[key] = newValue;
				//this.state[this.orderedKeys[i + 1]] = this.data[2][player1Index * slugSize + i + 1];
			...
			for (let j = 0; j < 22; j++){
				for (let k = 0; k < slugSize; k++){
					// Update current state
					let key = this.orderedKeys[slugSize + (j * slugSize) + k + 1];
					let newValue = this.data[2][j * slugSize + k + 1];
					this.state[key] = newValue;
	...
	server.on('error', (err) => {
		console.log(`F1_T04_Participants UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T04_Participants initialized--");
};


// to update every time new data comes in
F1_T04_Participants.prototype.generateRealtimeTelemetry = function () {
...
F1_T04_Participants.prototype.generateIntervalTelemetry = function () {
...
F1_T04_Participants.prototype.notify = function (point) {
...
F1_T04_Participants.prototype.listen = function (listener) {
...
F1_T04_Participants.prototype.SetFileTimestamp = function () {
...
F1_T04_Participants.prototype.clearHistoryObject = function (){
	console.log("Clear History Object - F1_T04_Participants");
...
F1_T04_Participants.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T04_Participants.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
  }
...
F1_T04_Participants.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T04_Participants_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T04_Participants_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		//server.send(command,54023, 'localhost')
		server.send(command,54023, '0.0.0.0')
...
module.exports = function () {
    return new F1_T04_Participants()
};
