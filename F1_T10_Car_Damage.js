// telemetry source object for the F1_T10_Car_Damage

const dgram = require('dgram');
const server = dgram.createSocket('udp4');
const fs = require('fs');
var F1_T10_Car_DamageCreateDict = require('../oci-telemetry-esports-F122-client/example/F1_T10_Car_Damage/F1_T10_Car_DamageCreateDict');
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T10 Car Damage");
...
function F1_T10_Car_Damage() {
	...
	//let rawDict = fs.readFileSync('../oci-telemetry-esports-F122-client/example/F1_T10_Car_Damage/F1_T10_Car_Damagedictionary.json')
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 10:");
			console.log(err);
			return;
		}
		...
		// Packet 10 is expected to be 660
		//console.log("Packet 10 length", this.data[2].length);
		if(this.data[2].length != 660){
			console.log("Rouge message length, packet 10:");
			return;
		}
		
		this.messageCount = this.messageCount + 1;
		if(this.messageCount % 60 == 0){
			console.log("    Packet 10 messages received: ", this.messageCount);
	...
	server.on('error', (err) => {
		console.log(`F1_T10_Car_Damage UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T10_Car_Damage initialized--");
};


// to update every time new data comes in
F1_T10_Car_Damage.prototype.generateRealtimeTelemetry = function () {
...
F1_T10_Car_Damage.prototype.generateIntervalTelemetry = function () {
...
F1_T10_Car_Damage.prototype.notify = function (point) {
...
F1_T10_Car_Damage.prototype.listen = function (listener) {
...
F1_T10_Car_Damage.prototype.SetFileTimestamp = function () {
...
F1_T10_Car_Damage.prototype.clearHistoryObject = function (){
	console.log("Clear History Object - F1_T10_Car_Damage");
...
F1_T10_Car_Damage.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T10_Car_Damage.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
  }
...
F1_T10_Car_Damage.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T10_Car_Damage_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T10_Car_Damage_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		//server.send(command,60023, 'localhost')
		server.send(command,60023, '0.0.0.0')
