// telemetry source object for the F1_T02_Lap_Data

const dgram = require('dgram');
const server = dgram.createSocket('udp4');
const fs = require('fs');
var F1_T02_Lap_DataCreateDict = require('../oci-telemetry-esports-F122-client/example/F1_T02_Lap_Data/F1_T02_Lap_DataCreateDict');
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T02 Lap Data");
...
function F1_T02_Lap_Data() {
	...
	//let rawDict = fs.readFileSync('../oci-telemetry-esports-F122-client/example/F1_T02_Lap_Data/F1_T02_Lap_Datadictionary.json')
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 02:");
			console.log(err);
			return;
		}
		...
		// Packet 02 is expected to be 530
		//console.log("Packet 2 length", this.data[2].length);
		if(this.data[2].length != 530){
			console.log("Rouge message length, packet 02: ", this.data[2].length);
			return;
		}
		
		this.messageCount = this.messageCount + 1;
		if(this.messageCount % 600 == 0){
			console.log("    Packet 02 messages received: ", this.messageCount);
		...
		// (packet_02, header, data, timestamp)
		//console.log(this.data[0]);
		if(this.data[0] == "packet_02"){
			...
			for (let l = 0; l < extraSlugSize; l++){
				// 23 X 24 = 552, 22 X 24 = 528
				// Update current state
				var key = this.orderedKeys[552 + l];
				//console.log("Start---", key);
				var newValue = this.data[2][528 + l];
				this.state[key] = newValue;
	...
	server.on('error', (err) => {
		console.log(`F1_T02_Lap_Data UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T02_Lap_Data initialized--");
};


// to update every time new data comes in
F1_T02_Lap_Data.prototype.generateRealtimeTelemetry = function () {
...
F1_T02_Lap_Data.prototype.generateIntervalTelemetry = function () {
...
F1_T02_Lap_Data.prototype.notify = function (point) {
...
F1_T02_Lap_Data.prototype.listen = function (listener) {
...
F1_T02_Lap_Data.prototype.SetFileTimestamp = function () {
...
F1_T02_Lap_Data.prototype.clearHistoryObject = function (){
	console.log("Clear History Object - F1_T02_Lap_Data");
...
F1_T02_Lap_Data.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T02_Lap_Data.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
  }
...
F1_T02_Lap_Data.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T02_Lap_Data_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T02_Lap_Data_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		//server.send(command,52023, 'localhost')
		server.send(command,52023, '0.0.0.0')
