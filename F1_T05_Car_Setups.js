// telemetry source object for the F1_T05_Car_Setups
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T05 Car Setups");
...
function F1_T05_Car_Setups() {
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 05:");
			console.log(err);
			return;
		}
		...
		// Packet 05 is expected to be 484
		//console.log("Packet 5 length", this.data[2].length);
		if(this.data[2].length != 484){
			console.log("Rouge message length, packet 05:");
			return;
		}

		this.messageCount = this.messageCount + 1;
		if(this.messageCount % 60 == 0){
			console.log("    Packet 05 messages received: ", this.messageCount);
	...
	server.on('error', (err) => {
		console.log(`F1_T05_Car_Setups UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T05_Car_Setups initialized--");
};


// to update every time new data comes in
F1_T05_Car_Setups.prototype.generateRealtimeTelemetry = function () {
...
F1_T05_Car_Setups.prototype.generateIntervalTelemetry = function () {
...
F1_T05_Car_Setups.prototype.notify = function (point) {
...
F1_T05_Car_Setups.prototype.listen = function (listener) {
...
F1_T05_Car_Setups.prototype.SetFileTimestamp = function () {
...
F1_T05_Car_Setups.prototype.clearHistoryObject = function (){
	console.log("Clear History Object - F1_T05_Car_Setups");
...
F1_T05_Car_Setups.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T05_Car_Setups.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
  }
...
F1_T05_Car_Setups.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T05_Car_Setups_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T05_Car_Setups_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		//server.send(command,55023, 'localhost')
		server.send(command,55023, '0.0.0.0')
