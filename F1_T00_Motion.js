// telemetry source object for the F1_T00_Motion

const dgram = require('dgram');
const server = dgram.createSocket('udp4');
const fs = require('fs');
var F1_T00_MotionCreateDict = require('../oci-telemetry-esports-F122-client/example/F1_T00_Motion/F1_T00_MotionCreateDict');
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T00 Motion");
...
function F1_T00_Motion() {
	...
	//let rawDict = fs.readFileSync('../oci-telemetry-esports-F122-client/example/F1_T00_Motion/F1_T00_Motiondictionary.json')
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 00:");
			console.log(err);
			return;
		}
		...
		// Packet 00 is expected to be 426
		//console.log("Packet 0 length", this.data[2].length);
		if(this.data[2].length != 426){
			console.log("Rouge message length, packet 00:");
			return;
		}
		...
		if(this.messageCount % 600 == 0){
			console.log("    Packet 00 messages received: ", this.messageCount);
		...
		if(this.data[0] == "packet_00"){
			...
			for (let j = 0; j < 22; j++){
				for (let k = 0; k < slugSize; k++){
					...
					//this.state[this.orderedKeys[slugSize + j * k]] = this.data[2][j * slugSize + k];
					this.state[key] = newValue;
			...
			for (let l = 0; l < extraSlugSize; l++){
				// 23 X 18 = 414, 22 X 18 = 396
				// Update current state
				var key = this.orderedKeys[414 + l];
				//console.log("Start---", key);
				var newValue = this.data[2][396 + l];
				this.state[key] = newValue;
	...
	server.on('error', (err) => {
		console.log(`F1_T00_Motion UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T00_Motion initialized--");
};


// to update every time new data comes in
F1_T00_Motion.prototype.generateRealtimeTelemetry = function () {
...
F1_T00_Motion.prototype.generateIntervalTelemetry = function () {
...
F1_T00_Motion.prototype.notify = function (point) {
...
F1_T00_Motion.prototype.listen = function (listener) {
...
F1_T00_Motion.prototype.SetFileTimestamp = function () {
...
F1_T00_Motion.prototype.clearHistoryObject = function (){
...
F1_T00_Motion.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T00_Motion.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
};
...
F1_T00_Motion.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T00_Motion_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T00_Motion_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		server.send(command,50023, '0.0.0.0')
