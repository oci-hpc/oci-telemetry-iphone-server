// telemetry source object for the F1_T01_Session

const dgram = require('dgram');
const server = dgram.createSocket('udp4');
const fs = require('fs');
var F1_T01_SessionCreateDict = require('../oci-telemetry-esports-F122-client/example/F1_T01_Session/F1_T01_SessionCreateDict');
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T01 Session");
...
function F1_T01_Session() {
	...
	//let rawDict = fs.readFileSync('../oci-telemetry-esports-F122-client/example/F1_T01_Session/F1_T01_Sessiondictionary.json')
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 01:");
			console.log(err);
			return;
		}
		...
		// Packet 01 is expected to be 530
		//console.log("Packet 1 length", this.data[2].length);
		if(this.data[2].length != 530){
			console.log("Rouge message length, packet 01:");
			return;
		}
		...
		if(this.messageCount % 60 == 0){
			console.log("    Packet 01 messages received: ", this.messageCount);
		...
		if(this.data[0] == "packet_01"){
			
			// Update state data
			header = this.data[1];
			headerPart1 = 4;
			headerPart2 = 5;
			headerSize = headerPart1 + headerPart2;
			...
			// Header data first, remember to skip packetID
			for (let i = 0; i < headerPart1; i++){
				// Update current state
				var key = this.orderedKeys[i];
				//console.log("Start---",key);
				var newValue = header[i];
			...
			for (let l = headerPart2; l < header.length; l++){
				// Update current state
				var key = this.orderedKeys[l-1];
				//console.log("Start---",key);
				var newValue = header[l];
			...
			// Other data
			for (let j = headerSize; j < this.orderedKeys.length; j++){
				// Update current state
				var key = this.orderedKeys[j];
				//console.log("Start---",key);
				var newValue = this.data[2][j - headerSize];
				this.state[key] = newValue;
	...
	server.on('error', (err) => {
		console.log(`F1_T01_Session UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T01_Session initialized--");
};


// to update every time new data comes in
F1_T01_Session.prototype.generateRealtimeTelemetry = function () {
...
F1_T01_Session.prototype.generateIntervalTelemetry = function () {
...
F1_T01_Session.prototype.notify = function (point) {
...
F1_T01_Session.prototype.listen = function (listener) {
...
F1_T01_Session.prototype.SetFileTimestamp = function () {
...
F1_T01_Session.prototype.clearHistoryObject = function (){
	console.log("Clear History Object - F1_T01_Session");
...
F1_T01_Session.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T01_Session.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
  }
...
F1_T01_Session.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T01_Session_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T01_Session_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		//server.send(command,51023, 'localhost')
		server.send(command,51023, '0.0.0.0')
