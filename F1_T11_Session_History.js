// telemetry source object for the F1_T11_Session_History

const dgram = require('dgram');
const server = dgram.createSocket('udp4');
const fs = require('fs');
var F1_T11_Session_HistoryCreateDict = require('../oci-telemetry-esports-F122-client/example/F1_T11_Session_History/F1_T11_Session_HistoryCreateDict');
...
async function asyncCleanHistoryOfDuplicates(obj) {
	...
	return new Promise((resolve, reject) => {
		console.log("History Clean of Duplicates - T11 Session History");
...
function F1_T11_Session_History() {
	...
	//let rawDict = fs.readFileSync('../oci-telemetry-esports-F122-client/example/F1_T11_Session_History/F1_T11_Session_Historydictionary.json')
    ...
    server.on('message', (msg, rinfo) => {
		...
		try{
			...
			console.log("Rouge message, packet 11:");
			console.log(err);
			return;
		}
		...
		// Packet 11 is expected to be 531
		//console.log("Packet 11 length", this.data[2].length);
		if(this.data[2].length != 531){
			console.log("Rouge message length, packet 11:");
			return;
		}
		
		this.messageCount = this.messageCount + 1;
		if(this.messageCount % 600 == 0){
			console.log("    Packet 11 messages received: ", this.messageCount);
		...
		if(this.data[0] == "packet_11"){
			
			// Update state data
			header = this.data[1];
			slugSize = 531; // 7 + 5x100 + 8x3
			carIndex = this.data[2][0];
			...
			for (let i = 0; i < slugSize; i++){
				...
				var key = this.orderedKeys[i + (carIndex * slugSize)];
				//console.log("Start---",key);
				var newValue = this.data[2][i];
	...
	server.on('error', (err) => {
		console.log(`F1_T11_Session_History UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("--F1_T11_Session_History initialized--");
};


// to update every time new data comes in
F1_T11_Session_History.prototype.generateRealtimeTelemetry = function () {
...
F1_T11_Session_History.prototype.generateIntervalTelemetry = function () {
...
F1_T11_Session_History.prototype.notify = function (point) {
...
F1_T11_Session_History.prototype.listen = function (listener) {
...
F1_T11_Session_History.prototype.SetFileTimestamp = function () {
...
F1_T11_Session_History.prototype.clearHistoryObject = function (){
	console.log("Clear History Object - F1_T11_Session_History");
...
F1_T11_Session_History.prototype.clearHistory = function (sessionUID, sessionTime, frameID){
...
F1_T11_Session_History.prototype.asyncStringyify = function (obj) {
	return new Promise((resolve, reject) => {
	  resolve(JSON.stringify(obj));
	});
  }
...
F1_T11_Session_History.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/F1_T11_Session_History_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/F1_T11_Session_History_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		//server.send(command,61023, 'localhost')
		server.send(command,61023, '0.0.0.0')
...
module.exports = function () {
    return new F1_T11_Session_History()
};
